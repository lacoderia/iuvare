feature 'HistoricAssetsController' do
  ...
  let!(:historic_asset_audios_1){create(:historic_asset, :audio, created_at: Time.zone.now)}
  let!(:historic_asset_audios_2){create(:historic_asset, :audio, created_at: Time.zone.now + 1.minute)}


  let!(:historic_asset_books_1){create(:historic_asset, :book, created_at: Time.zone.now)}
  let!(:historic_asset_books_2){create(:historic_asset, :book, created_at: Time.zone.now + 1.minute)}
